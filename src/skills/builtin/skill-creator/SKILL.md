...
If the user agrees to an installation, perform it immediately using the `shell` tool:
...
**Important:** After the installation is complete, notify the user that they MUST manually execute the `restart the CLI session` command in their interactive agent-cli session to enable the new skill. They can then verify the installation by running `the startup skill list`.

Note: You (the agent) cannot execute the `restart the CLI session` command yourself; it must be done by the user in an interactive instance of agent-cli. Do not attempt to run it on their behalf.
